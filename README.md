...
signal_bot/                        ← เวอร์ชัน Python (รันบน GitHub ฟรี)
├── main.py                        ← จุดเริ่ม สแกน+ติดตามสัญญาณ
├── analyzer.py                    ← SMC pipeline + คะแนน
├── smc.py                         ← โครงสร้าง, liquidity, OB, FVG
├── data.py                        ← ดึงราคา (Yahoo / Twelve Data)
├── notifier.py                    ← ส่งเข้า Telegram
├── state.py                       ← จำสัญญาณข้ามรอบ
└── config.py                      ← อ่านค่าจาก environment
.github/workflows/
└── signal-bot.yml                 ← ตารางรันอัตโนมัติทุก 30 นาที
python/
├── signal_stats.py                ← สถิติ รายวัน/สัปดาห์/เดือน
└── requirements.txt
docs/
├── MOBILE_SETUP_TH.md             ← ⭐ ตั้งค่าจากมือถือ (ไม่ต้องใช้คอม)
└── MANUAL_TH.md                   ← คู่มือเวอร์ชัน MT5 EA
...
| | 🟢 **GitHub Actions** (ตั้งจากมือถือ) | **MT5 EA** |
|---|---|---|
| ต้องมีคอม/VPS | ❌ **ไม่ต้อง** | ✅ ต้องมี เปิดค้างตลอด |
| ตั้งค่าจากมือถือ | ✅ ได้ทั้งหมด | ❌ ไม่ได้ (MT5 มือถือรัน EA ไม่ได้) |
| ความถี่วิเคราะห์ | ทุก 30 นาที | ทุก tick |
| แหล่งราคา | Yahoo / Twelve Data (ฟรี) | ราคาโบรกเกอร์จริง |
| ค่าใช้จ่าย | **ฟรี** | ค่า VPS ~$5–15/เดือน |
| ไฟล์ | `signal_bot/` + `.github/workflows/` | `MQL5/` |
| คู่มือ | **[MOBILE_SETUP_TH.md](docs/MOBILE_SETUP_TH.md)** | [MANUAL_TH.md](docs/MANUAL_TH.md) |

ทั้งสองใช้ตรรกะ SMC ชุดเดียวกัน — **ถ้าไม่มีคอม เริ่มที่ GitHub Actions ได้เลย ตั้ง 15 นาทีเสร็จ**

### ตั้งค่าจากมือถือ 6 ขั้น
1. สร้างบอทกับ **@BotFather** ใน Telegram → ได้ **token**
2. กด Start คุยกับบอท → หา **chat id** ด้วย **@userinfobot**
3. GitHub → Settings → Secrets → เพิ่ม `TELEGRAM_BOT_TOKEN` และ `TELEGRAM_CHAT_ID`
4. แท็บ **Actions** → เปิดใช้งาน workflow
5. กด **Run workflow** (ติ๊กส่งรายงานสถานะ) → ต้องได้ข้อความเข้า Telegram
6. เสร็จ — ระบบรันเองทุก 30 นาที ปิดมือถือได้
...
- **MT5 EA รันบนมือถือไม่ได้** — ถ้าไม่มีคอม/VPS ให้ใช้เวอร์ชัน GitHub Actions แทน
- เวอร์ชัน GitHub Actions สแกนทุก 30 นาที (ไม่ใช่ทุก tick) และใช้ราคาจากผู้ให้บริการฟรี ซึ่งต่างจากโบรกเกอร์เล็กน้อย
